...
void ProcessAllFunctions(std::string outputpath, const std::vector<kaddr> &functionObjects)
{
    ...
    {
        std::string funcName = UStruct::getCPPName(UObject::getOuter(funcObj)) + "::" + UObject::getName(funcObj);
        if (funcName.find("__Delegate") != std::string::npos || funcName.find("Default__") != std::string::npos)
            continue;

        std::string returnVal = "void";
        std::string params = "";

        functionFile << "\n// Function " << GetOuterFullName(funcObj) << std::endl
                     << "// 0x" << std::hex << UFunction::getFunc(funcObj) - Tools::lib_range.base << std::endl;

        for (kaddr funcParam = UStruct::getChildProperties(funcObj); funcParam; funcParam = FField::getNext(funcParam))
        {
        ...
        functionFile << returnVal << " " << funcName << "(" << params << ");" << std::endl;
    }
}
