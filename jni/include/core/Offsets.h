...
struct _Offsets32
{
    ...
    struct
    {
        ...
        kaddr BlockSizeBytes = Stride * FNameBlockOffsets;
        kaddr Lock = 0x0;               // 0x38
        kaddr CurrentBlock = 0x28;      // 0x4
        kaddr CurrentByteCursor = 0x2C; // 0x4
        kaddr Blocks = 0x30;            // 0x10000
    } FNameEntryAllocator;
    ...
    struct
    {
        kaddr PersistentLevel = 0x20; // 0x8
    } UWorld;

    struct
    {
        kaddr AActors = 0x70;     // 0x8
        kaddr ActorsCount = 0x74; // 0x4
    } ULevel;
    ...
    struct
    {
        kaddr ClassPrivate = 0xC;  // 0x8
        kaddr NamePrivate = 0x10;  // 0x8
        kaddr OuterPrivate = 0x18; // 0x8
    } UObject;

    struct
    {
        kaddr SuperStruct = 0x28;     // 0x8
        kaddr Children = 0x2C;        // 0x8
        kaddr ChildProperties = 0x30; // 0x8
        kaddr PropertiesSize = 0x34;  // 0x4
    } UStruct;

    struct
    {
        kaddr FunctionFlags = 0x6C; // 0x8
        kaddr Func = 0x88;          // 0x8
    } UFunction;
    ...
    struct
    {
        kaddr ClassPrivate = 0x4; // 0x8
        kaddr Next = 0x10;        // 0x8
        kaddr NamePrivate = 0x14; // 0x8
    } FField;

    struct
    {
        kaddr ElementSize = 0x24;     // 0x4
        kaddr PropertyFlags = 0x28;   // 0x8
        kaddr Offset_Internal = 0x34; // 0x4
        kaddr Size = 0x50;
    } FProperty;
    ...
    struct
    {
        kaddr Names = 0x2C;    // 0x16
        kaddr ArrayNum = 0x30; // 0x4
    } UEnum;                   // 不确定
    ...
    struct
    {
        kaddr FieldSize = 0x50;  // 0x1
        kaddr ByteOffset = 0x51; // 0x1
        kaddr ByteMask = 0x52;   // 0x1
        kaddr FieldMask = 0x53;  // 0x1
    } FBoolProperty;

    struct
    {
        kaddr PropertyClass = 0x50; // 0x8
    } FObjectProperty;

    struct
    {
        kaddr MetaClass = 0x54; // 0x8
    } FClassProperty;

    struct
    {
        kaddr InterfaceClass = 0x50; // 0x8
    } FInterfaceProperty;

    struct
    {
        kaddr Struct = 0x50; // 0x8
    } FStructProperty;

    struct
    {
        kaddr Inner = 0x50; // 0x8
    } FArrayProperty;

    struct
    {
        kaddr KeyProp = 0x50;   // 0x8
        kaddr ValueProp = 0x54; // 0x8
    } FMapProperty;

    struct
    {
        kaddr ElementProp = 0x50; // 0x8
    } FSetProperty;
